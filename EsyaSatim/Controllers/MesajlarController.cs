...
namespace EsyaSatim.Controllers
{
    public class MesajlarController : Controller
    {
        ...
        public ActionResult Cevapla(MesajlarYeni formData)
        {
            ...
            var kullanici_email = Database.Session.Query<Urunler>().Where(x => x.Id == Convert.ToInt32(formData.Gonderen_id)).ToList()[0].Email;
            var kullanici_id = Database.Session.Query<Kullanici>().Where(x => x.Email == kullanici_email).ToList()[0].Id;
            var mesaj = new Mesajlar()
            {
                Mesaj = formData.Mesaj,
                Gonderen_id = kullanici_id.ToString(),
                Alici_id = Session["Email"].ToString(),
                Tarih = DateTime.Now
            };
